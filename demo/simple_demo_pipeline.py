    ...
    while l_frame is not None:
        ...
        while l_user is not None:
            ...
            if(user_meta and user_meta.base_meta.meta_type==pyds.NvDsMetaType.NVDSINFER_TENSOR_OUTPUT_META): 
                ...
                output_bboxes = parse_objects_from_tensor_meta(layer)
                output_bboxes = output_bboxes[0]
        ...
        display_meta=pyds.nvds_acquire_display_meta_from_pool(batch_meta)   
        display_meta.num_rects = len(output_bboxes)
        # print(len(display_meta.rect_params))
        for i in range(16):
            # scale coordinates
            ...
            py_nvosd_rect_params.border_color.set(1.0, 0.0, 1.0, 1.0)
            py_nvosd_rect_params.border_width = 5
            py_nvosd_rect_params.has_bg_color = 0
            py_nvosd_rect_params.bg_color.set(0.0, 0.0, 0.0, 1.0)
            py_nvosd_rect_params.left = int(output_bboxes[i][0] * 640)
            py_nvosd_rect_params.top = int(output_bboxes[i][1] * 480)
            py_nvosd_rect_params.width = int(output_bboxes[i][2] * 640)
            py_nvosd_rect_params.height = int(output_bboxes[i][3] * 480)
...
def main(args, requested_pgie=None, config=None, disable_probe=False):
    ...
    if requested_pgie == "nvinferserver" and config != None:
        ...
        pgie.set_property('config-file-path', "../../config/occupancy_analytics/config_FastestDet.txt")
        # pgie.set_property('config-file-path', "../../config/occupancy_analytics/config_.txt")
