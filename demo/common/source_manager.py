...
class SRCM:
    ...
    def add(self, src):
        ...
        # print("after add src, total: ", len(self._pool))
    ...
    def alloc_index(self, src):
        if self._current_idx == self._max_src_num:
            self._current_idx = 0
        for i in range(self._max_src_num):
            if self._available_index[i] == True:
                ...
                logger.info("Source {} be bound with Index: source-{}".format(src.id, i))
                
                return True
        # print("fail to alloc index, maybe full.")
        logger.error("添加资源到资源管理器时失败. 资源数可能已满. fail to alloc index, maybe full.")
        return False
