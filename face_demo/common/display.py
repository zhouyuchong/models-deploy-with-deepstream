...
def dispaly_frame_pose(frame_meta, boxes, confs, kpts, step=3):
    # add_box_display_meta(frame_meta, batch_meta, boxes, confs,draw_boxes=True, draw_confs=True)
    add_pose_display_meta(frame_meta, kpts,
                          draw_kpts=True, draw_limbs=True)
...
def add_pose_display_meta(frame_meta, kpts, draw_kpts=True, draw_limbs=True):
    ...
    for kpt in kpts:
        ...
        if draw_kpts:
            ...
            for j,pt in enumerate(kpt):
                ...
                cnt = display_meta.num_rects-1
                circle_params = display_meta.rect_params[cnt]
                circle_params.left = int(pt[0]-5)
                circle_params.top = int(pt[1]-5)
                circle_params.width = 10
                circle_params.height = 10

                circle_params.border_width = 3
                circle_params.border_color.set(r,g,b,0.3)
                circle_params.has_bg_color = 1
                circle_params.bg_color.set(r,g,b,0.3)
                display_meta.num_rects += 1
