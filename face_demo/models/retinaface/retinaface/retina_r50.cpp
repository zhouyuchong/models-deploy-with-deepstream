    ...
    for (int b = 0; b < BATCH_SIZE; b++) {
        ...
        for (size_t j = 0; j < res.size(); j++) {
            ...
            for (int k = 0; k < 10; k += 2) {
                std::cout<<res[j].landmark[k]<<" "<<res[j].landmark[k + 1]<<std::endl;
            }
            ...
            for (int k = 0; k < 10; k += 2) {
                cv::circle(tmp, cv::Point(res[j].landmark[k], res[j].landmark[k + 1]), 1, cv::Scalar(255 , 0, 0), 1);
                std::cout<<res[j].landmark[k]<<" "<<res[j].landmark[k + 1]<<std::endl;
            }
            std::cout<<"======================"<<std::endl;
            // r = get_rect_adapt_landmark(tmp, INPUT_W, INPUT_H, res[j].bbox, lmk_ds);
            // cv::rectangle(tmp, r, cv::Scalar(0x27, 0xC1, 0x36), 2);
            //cv::putText(tmp, std::to_string((int)(res[j].class_confidence * 100)) + "%", cv::Point(r.x, r.y - 1), cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(0xFF, 0xFF, 0xFF), 1);
            // for (int k = 0; k < 10; k += 2) {
            //     cv::circle(tmp, cv::Point(res[j].landmark[k], res[j].landmark[k + 1]), 1, cv::Scalar(255 , 0, 0), 1);
            //     std::cout<<lmk_ds[k]<<" "<<lmk_ds[k + 1]<<std::endl;
            // }
